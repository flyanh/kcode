 ...
 *
 * @paper 6-22 针对于超大文件，mmap 效果不是很好，考虑块读多线程(ok 后面被证明了 mmap 效果更佳，至少对硬编码太多的读取)
 * @paper 6-25 封装的数据转换效率底下，准备全部把代码变成在线替换(ok 直接读取效率翻一倍)
 * @paper 6-27 进行调用者和响应者的硬编码，直接跳过方法名称寻找编号(ok 助力 2w)
 * @peper 6-28 进行循环展开(ok 提了一千分这样子，蚊子再小也是肉)
 * @perer 6-28 感觉多线程块读还是有优势？(ok 否决，块读多线程在现版本的确不比 mmap，猜测可能是因为 mmap 一直在跳字节，
 *                                       实际读取量并不多，但块读确实要实际读完的)
 * @paper 6-28 自己编写可扩容的 ArrayList(ok 提升明显，主要是可以直接使用 System.arraycopy 来进行拷贝合并碎片了)
 * @paper 6-28 优化计算部分的答案生成，将答案统一转换再生成一个 String(ok 助力 3w6)
 * @paper 6-28 优化 mmap buffer 的使用，不应该自己去管理偏移(OK 助力 3w68)
 * @peoer 6-28 优化整理碎片逻辑(ok)
 * @paper 6-28 二维数组查询速度比三维快多了(ok)
 * @paper 6-28 调用者和响应者在多线程过程中并不需要，快速跳过(ok)
 * @paper 6-28 ip 对实际上可以只编码 3 位(ok)
 * @paper 6-29 存储耗时使用 int 提升 5s+，来自于 java 虚拟机的神秘助力(ok)
 * @paper 6-29 优化计算第一阶段，不要每次都访问答案数组进行添加，而是先局部汇总最后整体添加(ok 助力 4w)
 * @paper 6-29 优化 ip 对的哈希编码运算，使用位运算，这样更快(ok 助力 4w2)
 * @paper 6-29 提前准备好每个 IP 对的字节数组，这样可以更快的生成答案(ok 线下怎么感觉慢了，经过测试，的确慢了...)
 * @paper 6-29 加入 Flyan 幸运数字(ok)
 * @paper 7-1 使用答案缓存机制(ok 如果没有答案缓存最高分 4w229)
 */
